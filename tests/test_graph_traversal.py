...
def main():
    ...
    graph_store.add_entity(
        "Theory of Relativity", entity_type="Theory", confidence=0.95
    )
    graph_store.add_entity(
        "Nobel Prize in Physics", entity_type="Award", confidence=0.95
    )
    graph_store.add_entity(
        "Princeton University", entity_type="Organization", confidence=0.9
    )
    ...
    graph_store.add_entity(
        "University of Paris", entity_type="Organization", confidence=0.9
    )
    ...
    paths = graph_store.path_query(
        start_entity="Albert Einstein", end_entity="Nobel Prize in Physics", max_hops=2
    )
    ...
    paths = graph_store.bidirectional_search(
        start_entity="Marie Curie", end_entity="Albert Einstein", max_hops=3
    )
