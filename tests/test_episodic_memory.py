...
class TestEpisodicMemoryStore:
    ...
    def test_recall_episodes_fts(self):
        ...
        assert any("neural" in r.summary.lower() or "neural" in r.title.lower() for r in results)

        # FTS search for "pasta"
        results = store.recall_episodes("pasta")
        assert len(results) >= 1
        assert any("pasta" in r.summary.lower() or "pasta" in " ".join(r.topics).lower() for r in results)
        store.close()
