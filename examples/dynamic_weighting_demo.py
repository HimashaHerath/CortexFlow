#!/usr/bin/env python3
"""
CortexFlow Dynamic Memory Weighting Test

This script tests the Dynamic Memory Weighting functionality in CortexFlow.
"""
...
def test_dynamic_weighting():
    ...
    parser = argparse.ArgumentParser(
        description="Test CortexFlow Dynamic Memory Weighting"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4000,
        help="Initial active tier token limit",
    )
    parser.add_argument(
        "--working-tokens",
        type=int,
        default=8000,
        help="Initial working tier token limit",
    )
    parser.add_argument(
        "--archive-tokens",
        type=int,
        default=12000,
        help="Initial archive tier token limit",
    )
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument(
        "--plot", action="store_true", help="Generate plots of tier allocations"
    )
    args = parser.parse_args()

    print(f"Testing CortexFlow Dynamic Memory Weighting with model {args.model}...")
    ...
    try:
        ...
        context_manager.add_message(
            "system", "You are a helpful AI assistant with dynamic memory capabilities."
        )
...
def print_tier_stats(stats):
    ...
    print(
        f"  Active tier: {tiers['active']['used']}/{tiers['active']['limit']} tokens "
        f"({tiers['active']['fullness'] * 100:.1f}% full)"
    )
    print(
        f"  Working tier: {tiers['working']['used']}/{tiers['working']['limit']} tokens "
        f"({tiers['working']['fullness'] * 100:.1f}% full)"
    )
    print(
        f"  Archive tier: {tiers['archive']['used']}/{tiers['archive']['limit']} tokens "
...
def plot_tier_allocations(context_manager):
    ...
    try:
        ...
        memory_stats = (
            context_manager.get_stats().get("memory", {}).get("tier_stats", {})
        )
        ...
        if memory_stats:
            ...
            for i, tier in enumerate(["active", "working", "archive"]):
                if tier in memory_stats and "usage_history" in memory_stats[tier]:
                    ...
                    plt.plot(
                        times, fullness, marker="o", label=f"{tier.capitalize()} Tier"
                    )
