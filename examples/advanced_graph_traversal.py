...
def main():
    ...
    config = CortexFlowConfig(use_graph_rag=True, knowledge_store_path=db_path)
    ...
    paths = graph_store.path_query(
        start_entity="Albert Einstein", end_entity="Nobel Prize in Physics", max_hops=3
    )
    ...
    paths = graph_store.bidirectional_search(
        start_entity="Marie Curie", end_entity="Albert Einstein", max_hops=4
    )
    ...
    contraction_stats = graph_store.contract_graph(
        min_edge_weight=0.2, min_confidence=0.3, combine_parallel_edges=True
    )
    ...
    abstraction_stats = graph_store.create_graph_abstraction(
        community_resolution=1.0, min_community_size=3
    )
    ...
    paths = graph_store.path_query_with_abstraction(
        start_entity="Albert Einstein", end_entity="Richard Feynman", max_hops=5
    )
