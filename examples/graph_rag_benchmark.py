#!/usr/bin/env python3
"""
Benchmark script for GraphRAG functionality in CortexFlow.
...
try:
    ...
    print(
        "Warning: requests library not available. Ollama integration will be disabled."
    )
...
from graph_rag_test import TEST_KNOWLEDGE  # noqa: E402
...
class GraphRAGBenchmark:
    ...
    def _load_knowledge(self):
        ...
        if graph_store.graph:
            ...
            if self.args.verbose:
                ...
                print(
                    f"  - SpaCy enabled: {hasattr(graph_store, 'nlp') and graph_store.nlp is not None}"
                )
                print(
                    f"  - NetworkX enabled: {hasattr(graph_store, 'graph') and graph_store.graph is not None}"
                )
        ...
        relation_added = graph_store.add_relation(
            "Guido van Rossum", "created", "Python", 1.0, {"test": True}
        )
        ...
        for i, knowledge in enumerate(TEST_KNOWLEDGE):
            ...
            fact_ids = self.manager.knowledge_store.remember_explicit(
                text=knowledge, source="benchmark_data", confidence=0.9
            )
        ...
        if graph_store.graph:
            ...
            if self.args.verbose and graph_store.graph.number_of_nodes() > 0:
                ...
                for entity_id, attrs in entities:
                    print(
                        f"  - ID {entity_id}: {attrs.get('name', 'Unknown')} ({attrs.get('entity_type', 'Unknown')})"
                    )
                ...
                if graph_store.graph.number_of_edges() > 0:
                    ...
                    for source, target, attrs in edges:
                        ...
                        source_name = graph_store.graph.nodes[source].get(
                            "name", "Unknown"
                        )
                        target_name = graph_store.graph.nodes[target].get(
                            "name", "Unknown"
                        )
    ...
    def _add_core_facts(self, graph_store):
        ...
        graph_store.add_relation(
            "Guido van Rossum", "created", "Python", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "Guido van Rossum", "worked", "Google", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "Google", "headquarters", "Mountain View", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "Mountain View", "city in", "Silicon Valley", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "Silicon Valley", "region in", "California", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "San Francisco", "city in", "California", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "Los Angeles", "city in", "California", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "Facebook", "developed", "PyTorch", 1.0, {"benchmark": True}
        )
        graph_store.add_relation(
            "PyTorch", "is a", "machine learning framework", 1.0, {"benchmark": True}
        )
    ...
    def evaluate_retrieval_precision(
        ...
        start_time = time.time()
        results = self.manager.knowledge_store.get_relevant_knowledge(
            query, max_results=5
        )
        ...
        if self.args.verbose:
            ...
            for i, result in enumerate(results):
                print(
                    f"    {i + 1}. {result.get('text', '')} [Score: {result.get('score', 0):.4f}, Type: {result.get('type', 'unknown')}]"
                )

            # Debug direct graph search results
            print("\n  DEBUG - Direct graph search results:")
            graph_results = self.manager.knowledge_store._graph_search(
                query, max_results=5
            )
    ...
    def evaluate_llm_answers(
        ...
        context = self.manager.knowledge_store.get_relevant_knowledge(
            query, max_results=5
        )
    ...
    def run_benchmarks(self):
        ...
        for query_type, queries in BENCHMARK_QUERIES.items():
            ...
            for i, query_data in enumerate(queries):
                ...
                # Evaluate precision
                precision, precision_details = self.evaluate_retrieval_precision(
                    query_type, query_data
                )
                precision_scores.append(precision)

                # Evaluate recall
                recall, recall_details = self.evaluate_retrieval_recall(
                    query_type, query_data
                )
                recall_scores.append(recall)

                # Evaluate path accuracy
                path_accuracy, path_details = self.evaluate_path_accuracy(
                    query_type, query_data
                )
                ...
                if self.args.evaluate_llm:
                    llm_accuracy, llm_details = self.evaluate_llm_answers(
                        query_type, query_data
                    )
            ...
            avg_path_accuracy = (
                np.mean(path_accuracy_scores) if path_accuracy_scores else 0
            )
            avg_timing = np.mean(timing_data) if timing_data else 0
            avg_hop_accuracy = (
                np.mean(hop_accuracy_scores) if hop_accuracy_scores else 0
            )
            avg_llm_accuracy = (
                np.mean(llm_answer_scores)
                if llm_answer_scores and self.args.evaluate_llm
                else 0
            )
        ...
        if self.args.evaluate_llm:
            overall_llm_accuracy = np.mean(
                list(self.results["llm_answer_accuracy"].values())
            )
    ...
    def _test_graph_traversal(self):
        ...
        try:
            ...
            for entity in entities:
                ...
                neighbors = graph_store.get_entity_neighbors(
                    entity["text"], direction="both"
                )
                print(f"    Found {len(neighbors)} neighbors:")
                for neighbor in neighbors[:3]:  # Show first 3
                    print(
                        f"      - {neighbor.get('entity', 'Unknown')} ({neighbor.get('relation', 'related_to')})"
                    )
            ...
            if conn is not None and (
                not hasattr(graph_store, "conn") or conn != graph_store.conn
            ):
                conn.close()
        ...
        try:
            ...
            if self.args.evaluate_llm:
                plt.bar(
                    [i + width * 2 for i in x],
                    [
                        self.results["llm_answer_accuracy"].get(qt, 0)
...
def main():
    ...
    parser.add_argument(
        "--graph-weight", type=float, default=0.3, help="Weight for graph-based results"
    )
    parser.add_argument(
        "--max-hops",
        type=int,
        default=3,
        help="Maximum path length for graph traversal",
    )
    parser.add_argument(
        "--db-path", default="benchmark_graph_rag.db", help="Database path"
    )
    parser.add_argument("--keep-db", action="store_true", help="Keep existing database")
    parser.add_argument(
        "--reload-knowledge",
        action="store_true",
        help="Force reload knowledge even if DB exists",
    )
    parser.add_argument("--output", help="Output file for benchmark results (JSON)")
    parser.add_argument("--verbose", action="store_true", help="Verbose output")
    parser.add_argument("--plot", action="store_true", help="Generate plots")
    parser.add_argument(
        "--evaluate-llm", action="store_true", help="Evaluate LLM answer accuracy"
    )
