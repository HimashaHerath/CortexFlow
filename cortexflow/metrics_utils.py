...
def calculate_precision(
    expected_entities: list[str], retrieved_texts: list[str]
) -> float:
    """
    ...
    for text in retrieved_texts:
        ...
        if any(
            expected in entities or any(expected in entity for entity in entities)
...
def calculate_recall(expected_entities: list[str], retrieved_texts: list[str]) -> float:
    ...
    found_count = 0
    for expected in normalized_expected:
        if expected in all_retrieved_entities or any(
            expected in entity for entity in all_retrieved_entities
        ):
            found_count += 1
...
def evaluate_hop_accuracy(expected_hops: int, actual_hops: int) -> float:
    ...
    hop_ratio = (
        min(actual_hops / expected_hops, expected_hops / actual_hops)
        if actual_hops > 0
        else 0
    )
    return hop_ratio
...
def evaluate_llm_answer(answer: str, expected_entities: list[str]) -> float:
    ...
    for entity in expected_entities:
        ...
        if (
            normalized_entity in normalized_answer
            or normalized_entity in answer_entities
            or any(normalized_entity in e for e in answer_entities)
        ):
            found_count += 1
...
def calculate_benchmark_metrics(
    query_results: list[dict[str, Any]], expected_data: dict[str, Any]
) -> dict[str, float]:
    """
